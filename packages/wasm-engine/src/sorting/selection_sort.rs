/// Selection Sort — instrumented for Algorithm Atlas WASM visualization.
///
/// For each position i, find the minimum in arr[i..n-1] and swap it into i.
/// Uses auxiliary_indices to highlight the current minimum candidate.
        ...
        for j in (i + 1)..n {
            ...
            if fb.arr[j] < fb.arr[min_idx] {
                fb.push(
                    ...
                    vec![min_idx],
                    format!(
                        "New minimum: {} at index {} (beats {} at index {})",
                        fb.arr[j], j, fb.arr[min_idx], min_idx
                    ),
                );
                min_idx = j;
            } else {
                fb.push(
                    ...
                    vec![min_idx],
                    format!(
                        "Comparing {} at {} with current min {} at {} — no change",
                        fb.arr[j], j, fb.arr[min_idx], min_idx
                    ),
                );
            }
