...
fn merge_step(fb: &mut FrameBuilder, frames: &mut Vec<SimulationFrame>, left: usize, mid: usize, right: usize) {
    let left_half  = fb.arr[left..mid].to_vec();
    let right_half = fb.arr[mid..right].to_vec();
    let auxiliary: Vec<usize> = (left..right).collect();

    fb.push(frames, None, None, auxiliary.clone(),
    ...
    while i < left_half.len() && j < right_half.len() {
        fb.comparisons += 1;
        let (li, rj) = (left + i, mid + j);
        if left_half[i] <= right_half[j] {
            fb.push(frames, Some([li, rj]), None, auxiliary.clone(),
                format!("Take {} from left (index {}) — {} ≤ {}", left_half[i], li, left_half[i], right_half[j]));
            fb.arr[k] = left_half[i];
            fb.swaps += 1;
            i += 1;
        } else {
            fb.push(frames, Some([li, rj]), None, auxiliary.clone(),
                format!("Take {} from right (index {}) — {} > {}", right_half[j], rj, left_half[i], right_half[j]));
            fb.arr[k] = right_half[j];
            fb.swaps += 1;
            j += 1;
        }
        fb.push(frames, None, Some([k, k]), auxiliary.clone(), format!("Placed at index {}", k));
        k += 1;
    }
    ...
    while i < left_half.len() {
        fb.arr[k] = left_half[i];
        fb.swaps += 1;
        fb.push(frames, None, Some([k, k]), auxiliary.clone(),
