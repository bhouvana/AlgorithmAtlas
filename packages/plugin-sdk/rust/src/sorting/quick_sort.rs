...
fn partition(fb: &mut FrameBuilder, frames: &mut Vec<SimulationFrame>, low: usize, high: usize) -> usize {
    let pivot = fb.arr[high];
    fb.push(frames, None, None, vec![high],
    ...
    for j in low..high {
        fb.comparisons += 1;
        if fb.arr[j] <= pivot {
            fb.push(frames, Some([j, high]), None, vec![high],
                format!("Compare arr[{}]={} ≤ pivot {} — will swap with store index {}", j, fb.arr[j], pivot, store));
            if store != j {
                fb.arr.swap(store, j);
                fb.swaps += 1;
                fb.push(frames, None, Some([store, j]), vec![high],
                    format!("Swapped index {} and {} — swap #{}", store, j, fb.swaps));
            }
            store += 1;
        } else {
            fb.push(frames, Some([j, high]), None, vec![high],
                format!("Compare arr[{}]={} > pivot {} — no swap", j, fb.arr[j], pivot));
        }
    }
    ...
    fb.push(frames, None, Some([store, high]), vec![store],
