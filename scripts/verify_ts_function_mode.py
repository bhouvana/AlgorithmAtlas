"""
Real, live verification of the TypeScript Function Mode adapter against the
6 already-migrated problems' REAL DB test cases (the same 240 typed cases
Python/JavaScript Function Mode already use -- never a second corpus, per
docs/atlascode-dual-run-modes.md's rule).

Mirrors verify_js_function_mode.py exactly, with TypeScript reference
solutions run through the real `tsx` subprocess (via run_typescript), not
asserted from code review:
  1. a correct reference TS solution -> must be Accepted (40/40)
  2. a deliberately wrong TS solution -> must fail at least one case
  3. one partial-credit solution -> must show a genuine mixed pass/fail count
  4. a missing-function solution -> must produce Function Contract Error

Run from repo root: python scripts/verify_ts_function_mode.py
"""
...
""",
    "two-sum-count-pairs": """
function count_pairs(nums: number[], target: number): number {
...
async def main() -> int:
    failures = []
    for pid, correct_code in CORRECT_TS.items():
        contract, cases = load_problem(pid)
        assert len(cases) == 40, f"{pid}: expected 40 cases, got {len(cases)}"

        result = await evaluate_function(correct_code, "typescript", contract, cases)
        ...
        wrong_code = WRONG_TS[pid]
        wrong_result = await evaluate_function(wrong_code, "typescript", contract, cases)
    ...
    for pid, partial_code in PARTIAL_TS.items():
        contract, cases = load_problem(pid)
        result = await evaluate_function(partial_code, "typescript", contract, cases)
        n_passed = sum(1 for r in result.case_results if r.passed)
    ...
    missing_result = await evaluate_function("function not_the_right_name() {}", "typescript", contract, cases[:1])
    ...
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        return 1
    print("All TypeScript Function Mode checks passed.")
    return 0
